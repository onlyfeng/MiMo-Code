...
> **2026-09-08 更新：** 以下は統合前のアーキテクチャ分析です。シェル・画像ツールの直接公開、制御ツールの除外、実行時レジストリ、添付非対応の説明は[現在の仕様](../codex-compact-tools.md)に置き換わりました。harness 判定を統一し、許可済み定義と公開名を別々に固定します。exec は上限付きで添付を転送します。
