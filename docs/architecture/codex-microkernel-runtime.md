...
> **2026-09-08 更新：** 下文保留早期架构分析，其中顶层 bash/图片工具、控制工具排除、实时注册表和不转发附件等描述已被本轮融合替代。当前行为与启用方法以[Codex 紧凑工具模式](../codex-compact-tools.md)为准；harness 解析已统一，完整授权池与可见名单分别冻结，exec 支持有界附件回传。
