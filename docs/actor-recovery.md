...
```text
actor resume <actor-id> [--task T1]
```

该命令继续原来的持久化用户任务，不接收替代任务文本。可选 `--task T1` 校验
已有同值绑定，或在原绑定缺失时验证实际任务来源、状态及 owner 后原子补齐。
已有不同绑定返回冲突，不静默覆盖；省略参数保持原绑定。
...
原中断 assistant 只在取得运行所有权并验证候选后，与可选任务补绑定在同一事务中结算。恢复期间收到的 inbox 消息
排队等待，恢复退出后由已有唤醒流程处理；恢复始终沿用原来的任务来源及 task。
自动压缩和内部重试仍可生成续行消息，最终 assistant 可以指向该内部续行消息；原外部
用户请求不会被替换。只有当前恢复流程成功提交的具体消息才能推进续行，同为 `hook`
来源、相同模型或相同 task 的其他消息也不能接管恢复。
