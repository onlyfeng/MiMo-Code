...
- Reviewed upstream `7641dbbd3b8aa20ffd4fb74089f2dc65bf032201..f11e35ede439df5555ca1f0309ffea8e9b49f06e`
  (one squash-merge commit; 17 paths, 1,280 insertions, 201 deletions from the accepted main tip).
  Upstream advanced again while the previous range was propagating into `dev/compat`, so this range was selected and propagated as its own pass.
- Prior fork main: `bcf2fba7c337a812071ccacc5184347c0bd5536e`; shared runtime/test snapshot `635618207270af55435aee88ce27003fc1809b44`.
- Bundled guidance content remains `c35c34d45a2e24ab6e48a7a3fd438d1c456352ed`; `src/tool/history.txt` and `src/tool/memory.txt` are runtime tool descriptions, not bundled skill content. No SDK/OpenAPI input or lockfile changed.
- The single selected capability is reconciled in [the synchronization inventory](upstream-sync-2026-09-11-f11e35ed.md). All nine active FD and sixteen active FC owners remain active; no owner retires and no unrelated consolidation is selected.
- C03 adopts bounded history search and `get`: SQL projections leave unused media payloads in the database, `get` returns paged `part_id` details, data-URL cleaning rejects mid-token prefixes and wrapped base64, FTS receives a neutral `[media <mime>]` marker, `around` keeps its anchor and reports truncation, and a one-shot `20260908000000_history_media_rebuild` migration clears the derived `history_fts` index for the resumable startup backfill.
- The merge was clean on every path. Sixteen of seventeen incoming paths had no fork divergence; `src/session/checkpoint.ts` is the only diverged file and its incoming change is the two copy lines redirecting verbatim recall to `around` then `get(part_id)`, which is factually correct in this fork.
- FC-002 keeps the checkpoint writer and FC-015 its budget — the marker changes length by a few characters inside an already-bounded field. FC-005's own migration shares the same date prefix; the fork already carries two such migrations and its journal regression checks the complete name set rather than a count, so no ordering or identity assumption breaks.
- One local failure is recorded rather than fixed: upstream's `test/history/details.test.ts > SQL preview bounds NUL-containing fields without losing get details` fails on darwin because the `length(CAST(... AS BLOB)) > 4000` preview guard does not fire for a NUL-prefixed value there. It reproduces on upstream's unmodified `f11e35ed` tree on the same machine and pinned Bun, and upstream's own ubuntu-latest `test` run for that SHA is green, so it is a platform difference rather than a fork integration defect. No assertion was relaxed.
- Pure registry commits do not advance the snapshot above; final branch-tip CI and remote ancestry are checked separately after publication.

## 2026-09-12 — actor execution/delivery model, models catalog availability, media reading
...
- Prior fork main: `4fa2402fdb51c1c5fcfacc5f4471a6c4005ea0cb`; shared runtime/test snapshot `bca24711ed37d3620cd7304603b81e98849642e5`.
...
## 2026-09-15 Inbox queue-to-transcript crash consistency

- Main source/test implementation: `74d4bfb6008071fca87c245c7791530660d64876`; starting main `370d12295f4c4628e10012ae46f3bcb454a5b587`, compat `49dce5816792e95050cd64561080d3257b714b7f`. Selected upstream remains `b4cc11cd652195af9a80297ed543218f3172e6c4`; this is a specified shared correction, with no compat policy promotion.
