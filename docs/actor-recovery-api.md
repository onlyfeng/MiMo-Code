...
事务提交前的请求取消会撤回尚未接管的恢复，不能补绑定任务或提前结算旧 assistant。事务提交后，Actor 已接管执行，即使调用方尚未收到 `202`；此时调用方断开不会释放正在执行的运行代。停止执行仍由 actor 原有取消、接收实例释放和生命周期收尾机制负责。API 没有增加独立的通用后台恢复入口。
