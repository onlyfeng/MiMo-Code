...
New HTTP endpoints:
- `GET /permission/ask-timeout` — returns current `permissionAskTimeoutMs`
- `POST /permission/ask-timeout` — body: `{ "ms": number | null }`, sets `permissionAskTimeoutMs`
...
- [x] T3: Server API — add GET/POST `/permission/ask-timeout` endpoints. acceptance: HTTP calls can read and write the timeout setting (covers: S2.7)
