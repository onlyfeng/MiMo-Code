...
- Second pushed tip `5e889b38` passed lint and typecheck CI; unit shard 4 exposed an asynchronous fixture lifetime error in `session-recovery.test.ts`: its helper returned `Instance.provide()` while `await using` disposed the temporary Git directory. CI showed `ENOENT` when project ID initialization wrote into the removed directory. Awaiting the operation before disposal fixes that fixture; the complete file passes 22/22 locally. A fresh final tip still requires exact-SHA CI.
