...
默认命令超时及输出预算分别为 10000 毫秒、10000 token。未知或错拼字段仍拒绝执行；
只有确定的候选会给出改写提示，不会自动修改字段，正确字段与错拼字段并存也不会忽略后者。
外层 `exec.timeout_seconds` 接受整数 1–600 秒，兼容上游的 `exec.timeout` 接受整数
1–600000 毫秒。例如 `{code: "return 1", timeout_seconds: 2}` 与
`{code: "return 1", timeout: 2000}` 使用相同计算预算；同时传入两个参数会报错。
均不传时默认 60 秒；等待工具不消耗计算时间，另有 30 分钟总时限。
