          ...
          ([key, mcp]) =>
            Effect.gen(function* () {
              ...
              if (
                ...
                (!fromHost && cfg.mcp_origins?.[key]?.type === "claude" && mcp.auto_connect !== true)
              ) {
          ...
          for (const name of new Set([...Object.keys(s.host), ...Object.keys(host), ...Object.keys(s.hostRetryAt)])) {
            ...
            const result =
              !mcp || !isMcpConfigured(mcp) || mcp.enabled === false
                ...
                : mcp.auto_connect === false ||
                    (!(name in host) && cfg.mcp_origins?.[name]?.type === "claude" && mcp.auto_connect !== true)
                  ? { status: { status: "pending" as const }, mcpClient: undefined, defs: undefined }
