...
When the resolved harness is `codex`, compact tools are automatic: hidden shell/file/task/skill/MCP calls go through `exec`. Authorized main and registered peer actors can also call the full declared Actor operations there, including resume for registered persistent full-context actors with a live original receiver; its optional task_id validates an existing binding or atomically fills a missing one from the original task namespace, and never overwrites a different binding; ordinary subagents remain limited to authorized sends to their real parent. `question` can be nested when interactive and authorized. Only a foreground main plan turn may use nested `plan_exit`: await all other calls first; approval commits the build continuation, terminates the script and switches the TUI, while decline or feedback lets the script continue. Build permissions are rebuilt next turn. Direct `actor`, `question` and `plan_exit` remain available when exposed; session and workflow controls stay direct. Harness priority is session → `MIMOCODE_CODEX_MODE` process override → inference. An explicit session `default` keeps native declarations; `MIMOCODE_ENABLE_EXEC_TOOL=true` only adds exec to a non-Codex harness and does not compact it. Nested calls retain their normal validators, permission checks and frozen membership. `tools.exec_command` uses `cmd`, with `yield_time_ms` as a terminating millisecond command timeout, not a background-terminal handle.
...
The trigger is `floor(effective window × ratio)`, with a default ratio of 90%, configurable through `MIMOCODE_COMPACTION_TRIGGER_RATIO`. The effective window is the model's prompt capacity (`limit.input` when the provider publishes one, else `limit.context`), optionally lowered by a valid `compaction.max_context` budget. Reserves still validate that budget but are not subtracted from the trigger. `mimo models <provider>` prints the resolved window and the trigger per model, and `/status` shows both plus the current usage. Two things users hit here: a model's usable window depends on the route (ChatGPT/Codex subscription vs direct API key vs a reseller like OpenRouter can all differ for the same model name, and a 1M catalog figure does not mean the route serves 1M), and providers may price very long prompts higher (OpenAI charges 2x input / 1.5x output for GPT-5.6 requests above 272K input) — `compaction.max_context` is the knob for both.
