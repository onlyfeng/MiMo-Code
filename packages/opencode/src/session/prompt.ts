...
import { toolPresentationProgress } from "../mcp/tool-progress"
import { LSP } from "../lsp"
...
export interface Interface {
  ...
  readonly resumeMainCascading: (input: ResumeTurnInput) => Effect.Effect<void, InstanceType<typeof NotFoundError> | Session.BusyError | Session.RecoveryConflictError>
  ...
  readonly resumeBackground: (input: ResumeTurnInput) => Effect.Effect<void, InstanceType<typeof NotFoundError> | Session.BusyError | Session.RecoveryConflictError>
  ...
  readonly sweepOrphanToolParts: (
    sessionID: SessionID,
    opts?: { before?: number; ownedMessageIds?: ReadonlySet<string>; idleAtAdmission?: boolean },
  ) => Effect.Effect<void>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cascadeAdmissions = new Map<SessionID, Set<AbortController>>()
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      ...
      for (const admission of cascadeAdmissions.get(sessionID) ?? []) admission.abort()
      ...
      yield* Effect.forEach(
        ...
        (actor) =>
          Effect.gen(function* () {
            yield* (boundActor ?? spawnRef.current)?.markGroupAbort?.(sessionID, actor.actorID) ?? Effect.void
      ...
      yield* Effect.forEach(
        ...
        (actor) =>
          Effect.gen(function* () {
            const svc = boundActor ?? spawnRef.current
            if (svc) {
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
        ...
        const executeMcp = (
          ...
          run.promise(
            Effect.gen(function* () {
              ...
              if (modelFacing && useMcpToolSearch && !useGPTTools && !loadedMcpTools.has(key)) {
                return yield* Effect.fail(
              ...
              const progress = toolPresentationProgress(opts.experimental_context)
              const result: Awaited<ReturnType<NonNullable<typeof execute>>> = yield* Effect.tryPromise({
                try: () => execute(mcpBeforeOutput.args, {
                  ...opts,
                  experimental_context: {
                    onMcpToolProgress: async (meta: Record<string, unknown>) => {
                      Object.assign(presentation, meta)
                      progress.update({ _meta: meta })
                      await Promise.all([
                        progress.drain(),
                        run.promise(
              ...
              const metadata = {
                ...
                ...(Object.keys(presentation).length
                  ? { mcp: { ...normalized.metadata.mcp, _meta: { ...presentation, ...normalized.metadata.mcp._meta } } }
                  : {}),
    ...
    const sweepOrphanToolParts = Effect.fn("SessionPrompt.sweepOrphanToolParts")(function* (
      ...
      if (!owned && !(opts?.idleAtAdmission ?? (yield* status.get(sessionID)).type === "idle")) return
    ...
    const idleSweep = (sid: SessionID, opts?: { before?: number; ownedMessageIds?: ReadonlySet<string>; idleAtAdmission?: boolean }) =>
    ...
    const promptWork = Effect.fnUntraced(function* (
      ...
      const session = yield* sessions.get(input.sessionID)
      if (hookNonTextRequiresProvenance(input)) {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.prompt")(
      function* (input: PromptInput) {
        return yield* promptWork(input, (next) => runSharedLoop(next, true))
      },
    )
    ...
    const startPrompt = Effect.fn("SessionPrompt.startPrompt")(function* (input: PromptInput) {
      ...
      return yield* state.startRunning(
        ...
        promptWork(
          input,
          (next) =>
            runLoop(next.sessionID, next.agentID, next.titleLocale, undefined, undefined, undefined, undefined, next.deferInbox, false, next.source).pipe(
              Effect.exit,
    ...
    const recovery: Interface["recovery"] = Effect.fn("SessionPrompt.recovery")(function* (input) {
      ...
      return (yield* recoveryCandidates(input.sessionID, actorID)).filter((item) => item.kind === "assistant")
    })
    ...
    const runLoop: (
      ...
      const hintToken: MainHintToken = resolvedAgentID === "main" && turnSource === "user"
        ? openMainHintToken(sessionID, ctx.directory)
        : { cancelled: true }
      ...
      const firePostSession = (exit: Exit.Exit<MessageV2.WithParts, unknown>) =>
        Effect.gen(function* () {
          ...
          if (outcome === "completed" && resolvedAgentID === "main") {
            ...
            yield* Effect.gen(function* () {
              try {
                ...
                const followUp = Effect.gen(function* () {
                  ...
                  const source = sliceMsgs.find((message) => message.info.id === finalAsst.parentID)?.info
                  if (!source || source.role !== "user" || source.source !== "user") return undefined as never
                  if (!isPendingHintLive(pending)) return undefined as never
                  const msgId = MessageID.ascending()
                  const committed = yield* sessions.commitUserMessageIfLatest({
                    expectedUserID: source.id,
                    shouldCommit: () => isPendingHintLive(pending),
                    message: {
                      id: msgId, sessionID, role: "user", agentID: resolvedAgentID,
                      time: { created: Date.now() }, agent: agentName,
                      model: model ?? source.model, task_id: source.task_id, source: "hook",
                    },
                    parts: [{ id: PartID.ascending(), messageID: msgId, sessionID,
                      type: "text", text: buildHintText(statusOut), synthetic: true }],
                  })
                  if (!committed) return undefined as never
                  ...
                  const result = yield* runLoop(sessionID, resolvedAgentID, titleLocale, undefined, undefined, undefined, undefined, false, false, "hook")
                  return result
            ...
            }).pipe(RunApproval.provide(undefined), Effect.forkIn(scope), Effect.ignore)
          }
        }).pipe(Effect.ignore)
      ...
      return yield* Effect.gen(function* () {
        ...
        while (true) {
          ...
          // hasMemoryOrTasks. Persist once per user to keep reload order stable.
          const lastUserMsgForRecall = msgs.findLast(
            (m): m is MessageV2.WithParts & { info: MessageV2.User } => m.info.role === "user",
          )
          if (
            lastUserMsgForRecall &&
            shouldInjectActiveRecallReminder(lastUserMsgForRecall.info) &&
            !hasSyntheticReminder(lastUserMsgForRecall.parts, RECALL_REMINDER_MARKER)
          ) {
        ...
        Effect.ensuring(MCP.releaseTurnClients(mcpContext)),
        Effect.catchCause((cause) =>
    ...
    const runSharedLoop = Effect.fn("SessionPrompt.runSharedLoop")(function* (input: z.infer<typeof LoopInput>, joinRunning = false) {
      const agentID = input.agentID ?? "main"
      let started = false
          ...
          : Effect.gen(function* () {
              while (true) {
                ...
                const result = yield* state
                  .ensureRunning(input.sessionID, agentID, lastAssistant(input.sessionID, agentID), work, joinRunning || !!input.inboxID)
                  .pipe(Effect.exit)
        ...
        const results = yield* Effect.promise(() =>
          Promise.all(
            shellMatches.map(async ([, cmd]) =>
              capSyntheticText(
                (await Process.text([cmd], { shell: sh, nothrow: true })).text,
        ...
        commandWork(
          ...
          (next) =>
            promptWork(
              next,
              (loopInput) =>
                runLoop(loopInput.sessionID, loopInput.agentID, loopInput.titleLocale, undefined, undefined, undefined, undefined, loopInput.deferInbox, false, loopInput.source).pipe(
                  Effect.exit,
                  Effect.map((exit) => ({ started: true, exit })),
                ),
              idle,
    ...
    const startResumeTurn = Effect.fn("SessionPrompt.startResumeTurn")(function* (input: {
      ...
      yield* state.assertNotBusy(input.sessionID, input.actorID)
      const candidates = yield* recoveryCandidates(input.sessionID, input.actorID)
      const candidate = input.userMessageID
        ? candidates.find((item) => item.kind === "parent-user" && item.userMessageID === input.userMessageID)
        : input.assistantMessageID
          ? candidates.find((item) => item.kind === "assistant" && item.assistantMessageID === input.assistantMessageID)
          : candidates.at(-1)
      if (!candidate || (input.actorID !== "main" && candidate.kind !== "assistant"))
        return yield* Effect.fail(new NotFoundError({ message: input.userMessageID ? "No resumable trailing user found for message " + input.userMessageID : "No resumable interrupted turn found for " + (input.assistantMessageID ?? input.actorID) }))
      const assistantMessageID = candidate.kind === "assistant" ? candidate.assistantMessageID : undefined
      const parentMessageID = candidate.kind === "assistant" ? candidate.parentMessageID : candidate.userMessageID
      const strictTail = candidate.kind === "parent-user"
      ResumeTestHooks.onPlanResolved?.({
        action: strictTail || !MessageV2.hasUsefulAssistantParts(MessageV2.parts(assistantMessageID!)) ? "user-resume" : "tool-resume",
        assistantMessageID,
        parentMessageID,
      })
      if (input.exclusiveHook && ResumeTestHooks.beforeExclusiveOccupy) yield* ResumeTestHooks.beforeExclusiveOccupy()
      ...
      const abandon = Effect.suspend(() =>
        !strictTail && recovered.redispatch && recovered.parentID
      ...
      const validate = Effect.gen(function* () {
        if (ResumeTestHooks.beforeAdmissionRecheck) yield* ResumeTestHooks.beforeAdmissionRecheck()
        ...
        if (input.validate) {
          ...
          if (
            !current.some(
              (item) =>
                item.kind === "assistant" && item.assistantMessageID === assistantMessageID &&
        ...
        yield* sessions.commitRecoveryCandidate({
          ...
          onCommitted: ({ redispatch }) => {
            ...
            recovered.id = assistantMessageID
            recovered.parentID = parentMessageID
            input.onCommitted?.()
          },
        })
        if (input.onAdmitted) yield* input.onAdmitted
      })
      const owned = yield* state.startOwned(
        input.sessionID,
        input.actorID,
        ...
        validate.pipe(
          ...
          Effect.orDie,
          Effect.andThen(Effect.suspend(() => ResumeTestHooks.afterAdmissionBeforeCleanup?.() ?? Effect.void)),
          Effect.andThen(
            Effect.suspend(() =>
              runLoop(
                input.sessionID,
                ...
                recovered.redispatch ? "user" : "hook",
                recovered.redispatch ? recovered.parentID : undefined,
                strictTail,
              ),
            ).pipe(
              Effect.ensuring(
      ...
      if (Cause.hasInterruptsOnly(admission.cause)) return yield* Effect.failCause(admission.cause)
      const error = Cause.squash(admission.cause)
      if (error instanceof NotFoundError || error instanceof Session.RecoveryConflictError) return yield* Effect.fail(error)
      return yield* Effect.fail(new NotFoundError({ message: "Resume admission did not complete" }))
    })

    const startResume = Effect.fn("SessionPrompt.startResume")(function* (input: ResumeTurnInput, exclusiveHook = false) {
      if (input.agentID && input.agentID !== "main") return yield* Effect.fail(new NotFoundError({ message: "Actor recovery requires its owning lifecycle" }))
      const epoch = cascadeEpochBySession.get(input.sessionID) ?? 0
      const completion = yield* startResumeTurn({
        ...input,
        exclusiveHook,
        actorID: "main",
      ...
      yield* cascadeSubagentResume(input.sessionID, epoch).pipe(Effect.ignoreCause, Effect.forkIn(scope))
      return completion
    })
    ...
    const resume = Effect.fn("SessionPrompt.resume")(function* (input: ResumeTurnInput) {
      return yield* yield* startResume(input, true)
    })

    // Reuse Actor.resume's retained-context, generation, task and cancellation
    // admission. Registry-only or released-context rows never gain a new owner.
    const cascadeSubagentResume: Interface["cascadeSubagentResume"] = Effect.fn("SessionPrompt.cascadeSubagentResume")(
      function* (sessionID, epoch = cascadeEpochBySession.get(sessionID) ?? 0) {
        const outcomes: { actorID: string; status: "resumed" | "skipped" | "failed"; reason?: string }[] = []
        for (const actor of yield* actorRegistry.listBySession(sessionID)) {
          if (actor.actorID === "main" || actor.mode === "peer") continue
          ...
          if (actor.status !== "idle" || (yield* executions.current(sessionID, actor.actorID))) {
            outcomes.push({ actorID: actor.actorID, status: "skipped", reason: "live" })
            continue
          }
          const service = boundActor ?? spawnRef.current
          const candidates = yield* (service?.recovery?.({ sessionID, actorID: actor.actorID }) ?? Effect.succeed([]))
            .pipe(Effect.catch(() => Effect.succeed([])))
          const latest = candidates.at(-1)
          if (!latest || latest.kind !== "assistant" || !service?.resume) {
            outcomes.push({ actorID: actor.actorID, status: "skipped", reason: "no-recovery-candidate" })
            continue
          }
          const controller = new AbortController()
          const pending = cascadeAdmissions.get(sessionID) ?? new Set<AbortController>()
          pending.add(controller)
          cascadeAdmissions.set(sessionID, pending)
          if ((cascadeEpochBySession.get(sessionID) ?? 0) !== epoch) controller.abort()
          const admission = yield* service.resume({
            sessionID,
            actorID: actor.actorID,
            ...
            Effect.onExit((exit) => Exit.isFailure(exit) ? Effect.sync(() => controller.abort()) : Effect.void),
            Effect.exit,
            Effect.ensuring(Effect.sync(() => {
              pending.delete(controller)
              if (pending.size === 0) cascadeAdmissions.delete(sessionID)
            })),
          )
          outcomes.push({ actorID: actor.actorID, status: Exit.isSuccess(admission) ? "resumed" : "failed",
            ...(Exit.isFailure(admission) ? { reason: "not-admitted" } : {}) })
        }
        return outcomes
      },
    )

    const resumeMainCascading: Interface["resumeMainCascading"] = Effect.fn("SessionPrompt.resumeMainCascading")(
      function* (input) {
        yield* startResume(input)
      },
    )
    ...
    const impl = Service.of({
      ...
      recovery,
      startResume,
      resumeBackground: (input) => startResume(input).pipe(Effect.asVoid),
      startActorResume,
      resume,
      startSummarize,
