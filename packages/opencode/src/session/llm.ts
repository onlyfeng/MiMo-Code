...
/**
 * Resolve the exact provider-facing descriptor shape for an already-selected
 * wire tool set. The AI SDK's schema wrapper may expose its JSON schema through
 * a getter, while execute/lifecycle callbacks must never contribute to request
 * sizing. Keep that trusted getter boundary here beside the real dispatch
 * filter so preflight and streamText observe the same membership and schemas.
 */
export async function materializeWireToolDescriptors(tools: Record<string, Tool>) {
  return Promise.all(
    Object.entries(tools).map(async ([name, item]) => {
      ...
      return {
        ...
        inputSchema: await Promise.resolve(asSchema(item.inputSchema).jsonSchema),
        ...(item.inputExamples === undefined ? {} : { inputExamples: item.inputExamples }),
