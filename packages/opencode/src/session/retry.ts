...
export const NETWORK_MAX_RETRIES = STREAM_MAX_RETRIES
...
export function resolve(config: RetryConfigSource | undefined, providerID?: string): ResolvedRetryConfig {
  ...
  return {
    request: configuredBudget(DEFAULT_RETRY_CONFIG.request, global?.request, provider?.request),
    stream: configuredBudget(DEFAULT_RETRY_CONFIG.stream, global?.stream, provider?.stream),
...
export function budgetFor(config: ResolvedRetryConfig, decision: RetryDecision): RetryBudget {
  if (decision.scope === "request") return config.request
...
export function decide(
  ...
  const retry = (kind: RetryKind, retryPhase = phase, retryMessage = message): RetryDecision => ({
    retryable: true,
    phase: scope === "request" ? "request" : retryPhase,
    scope,
    kind,
  ...
  // 按 message / metadata.code 归回 network；request/candidate/judge 仍使用各自的有界预算。
  if (
    MessageV2.APIError.isInstance(error) &&
  ...
  //   request scope → request budget (default 4 / 30s, including network/server/rate-limit);
