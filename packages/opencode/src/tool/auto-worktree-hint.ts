...
export function findGitMainWorktree(startDir: string): string | null {
  ...
  // A negative lookup is not stable: this same directory may become a repo
  // after `git init` later in the session. Cache positive layouts only.
...
function completedWritePaths(part: MessageV2.ToolPart): string[] {
  ...
  if (part.tool === "apply_patch" && Array.isArray(metadata.files)) {
    return metadata.files.flatMap((value) => {
      ...
      if (!file) return []
      return [file.filePath, file.movePath].filter((item): item is string => typeof item === "string" && item.length > 0)
    })
  }
  ...
  for (const message of messages) {
    for (const part of message.parts) {
      ...
      if (FILE_WRITE_TOOLS.has(part.tool)) {
        const completed = completedWritePaths(part)
