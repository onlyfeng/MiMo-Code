...
describe("Runner", () => {
  ...
  it.live(
    "cancelDetached can release a signalled ordinary cancellation without awaiting cleanup",
    Effect.gen(function* () {
      ...
      yield* runner.start(Effect.never.pipe(Effect.ensuring(
        Deferred.succeed(entered, undefined).pipe(Effect.andThen(Deferred.await(release))),
      )))
      const cancelling = yield* runner.cancel.pipe(Effect.forkChild)
      yield* Deferred.await(entered)
      expect(runner.state._tag).toBe("Cancelling")
      yield* runner.cancelDetached.pipe(Effect.timeout("1 second"))
      expect(runner.busy).toBe(false)
      const next = yield* runner.startOwned(Effect.never)
      yield* Deferred.succeed(release, undefined)
      yield* Fiber.join(cancelling)
      expect(runner.state._tag === "Running" && runner.state.run.id).toBe(next.runId)
