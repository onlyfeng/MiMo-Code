...
describe("permission ask timeout (real clock)", () => {
  ...
  test.each(["1.5", "Infinity", "NaN", "0", "-1"])(
    "ignores a non-positive-integer env timeout (%s)",
    async (value) => {
      await using tmp = await tmpdir()
      const previous = process.env.MIMOCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS
      process.env.MIMOCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS = value
      try {
        await Instance.provide({
          directory: tmp.path,
          fn: async () =>
        ...
        if (previous === undefined) delete process.env.MIMOCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS
        else process.env.MIMOCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS = previous
      }
    },
    10000,
  )
