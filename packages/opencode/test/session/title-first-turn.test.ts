  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        // Compat receipts retain part identity; later new messages use the ID-free template.
        const receipt = { ...input, parts: input.parts.map((part) => ({ ...part, id: PartID.ascending() })) }
        await run(SessionPrompt.Service.use((svc) => svc.prompt(receipt)))
        ...
        release()
        await run(SessionPrompt.Service.use((svc) => svc.prompt(receipt)))
        const persisted = await run(Session.Service.use((svc) => svc.messages({ sessionID: session.id })))
        expect(persisted).toHaveLength(1)
        expect(persisted[0].parts.map((part) => part.id).sort()).toEqual(receipt.parts.map((part) => part.id).sort())
        await Bun.sleep(150)
