...
describe("session.retry.retryable", () => {
  ...
  test("keeps bounded network mode finite when maxRetries is omitted", () => {
    expect(SessionRetry.resolve({ retry: { network: { mode: "bounded" } } }).network).toMatchObject({
      mode: "bounded",
      maxRetries: SessionRetry.NETWORK_MAX_RETRIES,
    })
  })

  test("applies top-level jitter defaults with budget and provider overrides", () => {
    const global = SessionRetry.resolve({ retry: { jitterRatio: 0, stream: { jitterRatio: 0.2 } } })
    expect(global.request.jitterRatio).toBe(0)
    expect(global.server.jitterRatio).toBe(0)
    expect(global.stream.jitterRatio).toBe(0.2)

    const provider = SessionRetry.resolve(
      {
        retry: { jitterRatio: 0 },
        provider: { test: { retry: { jitterRatio: 0.3, network: { jitterRatio: 0.4 } } } },
      },
      "test",
    )
    expect(provider.request.jitterRatio).toBe(0.3)
    expect(provider.server.jitterRatio).toBe(0.3)
    expect(provider.network.jitterRatio).toBe(0.4)
  })
    ...
    const errors = [
      {
        error: new MessageV2.APIError({
          ...
          responseBody: JSON.stringify({
            type: "error",
            error: { type: "upstream_error", code: "stream_read_error", message: "upstream failed" },
          }),
        }).toObject(),
        kind: "stream",
      },
      { error: new Error("SSE read timed out"), kind: "network" },
    ] as const

    for (const { error, kind } of errors) {
      const decision = SessionRetry.decide(error, "request", "request")
      expect(decision).toMatchObject({ retryable: true, phase: "request", scope: "request", kind })
      expect(SessionRetry.budgetFor(resolved, decision)).toBe(resolved.request)
    }
  })

  test("keeps MaxMode network retries inside candidate and judge budgets", () => {
    const resolved = SessionRetry.resolve(undefined, "test")
    expect(
    ...
    ).toMatchObject({ mode: "bounded", maxRetries: 3, maxElapsedMs: 3 * 60_000 })
    expect(
      SessionRetry.budgetFor(resolved, {
        retryable: true,
        phase: "stream",
