...
async function addCheckpointPart(sessionID: SessionID, messageID: MessageID, coveredUpTo: MessageID) {
  await svc.updatePart({
    id: PartID.ascending(),
    sessionID,
    messageID,
...
describe("MessageV2.filterCompacted", () => {
  test("returns all messages when no checkpoint", async () => {
    await Instance.provide({
      directory: root,
      fn: async () => {
        const session = await svc.create({})
  ...
  test("stops at checkpoint boundary and returns chronological order", async () => {
    await Instance.provide({
      directory: root,
      fn: async () => {
        const session = await svc.create({})

        // Chronological: u1, a1, u2(+checkpoint part), a2
        // Stream (newest first): a2, u2(checkpoint -> break)
        ...
        const result = MessageV2.filterCompacted(MessageV2.stream(session.id))
        expect(result.map((item) => item.info.id)).toEqual([u2, a2])
  ...
  test("prefers latest checkpoint boundary when repeated checkpoints exist", async () => {
    await Instance.provide({
      directory: root,
      fn: async () => {
        const session = await svc.create({})
