      ...
      fn: async () =>
        AppRuntime.runPromise(
          Effect.gen(function* () {
            ...
            yield* prompt
              .startResume({ sessionID: session.id, assistantMessageID: shell2.id })
      ...
      fn: async () =>
        AppRuntime.runPromise(
          Effect.gen(function* () {
            ...
            yield* prompt
              .startResume({ sessionID: session.id, assistantMessageID: useful.id })
              .pipe(Effect.catch(() => Effect.void))
            yield* Effect.sleep("200 millis")
            const after = yield* sessions.messages({ sessionID: session.id, agentID: "main" })
            return {
      ...
      fn: async () =>
        AppRuntime.runPromise(
          Effect.gen(function* () {
            ...
            const candidates = yield* prompt.recovery({ sessionID: session.id, agentID: "main" })
            yield* prompt
              .startResume({ sessionID: session.id, assistantMessageID: assistant.id })
              .pipe(Effect.catch(() => Effect.void))
