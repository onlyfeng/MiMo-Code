...
describe("buildLLMRequestPrefix", () => {
      ...
      fn: async () => {
        ...
        const prefix = await AppRuntime.runPromise(
          buildLLMRequestPrefix({
            ...
            msgs: await AppRuntime.runPromise(SessionNs.Service.use((svc) => svc.messages({ sessionID: session.id }))),
            additions: [],
          }),
        )
        ...
        expect(prefix.activeTools).toContain(identity.codex ? "exec" : "read")
        if (identity.codex) {
          expect(prefix.tools.bash).toBeDefined()
          expect(prefix.activeTools).not.toContain("bash")
          expect(prefix.activeTools).not.toContain("apply_patch")
        }
      },
    })
  })
  ...
  test("frozen full-context tools honor session permission and message-level disables", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const messageDisabled = await AppRuntime.runPromise(buildLLMRequestPrefix({ ...input, permission: [] }))
  ...
  test.each(["default", "codex"] as const)(
    "frozen full-context tools record loaded MCP membership for %s without losing the captured search pool",
    async (harness) => {
      await using tmp = await tmpdir({ git: true })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          expect(prefix.activeTools.includes("mcp_tool_search")).toBe(harness === "default")
          expect(prefix.activeTools.includes("mcp_visible")).toBe(harness === "default")
          expect(prefix.activeTools).not.toContain("mcp_late")
        },
      })
    },
  )
