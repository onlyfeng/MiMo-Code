...
test("raw ETIMEDOUT Error retains the bounded request scope", () => {
  const err = Object.assign(new Error("connect ETIMEDOUT"), { code: "ETIMEDOUT" })
  const decision = decide(err, "request", "request")
  expect(decision.retryable).toBe(true)
  expect(decision.kind).toBe("network")
  expect(budgetFor(resolve(undefined), decision)).toEqual(resolve(undefined).request)
  expect(budgetFor(resolve(undefined), decision).mode).toBe("bounded")
})
