// [TP-MCU-R7-21][TP-MCU-R10-20] Host admission: removed-host failure must not clear user connection.
...
it.live("[TP-MCU-R7-21][TP-MCU-R10-20] removed host failure must not clear current user connection", () =>
  Effect.gen(function* () {
    let release!: () => void
    let entered!: () => void
    let oldLists = 0
      ...
      Effect.sync(() =>
        Bun.serve({
          ...
          async fetch(request) {
            ...
            const name = new URL(request.url).pathname === "/old" ? "old" : "user"
            if (msg.method === "tools/list" && name === "old" && ++oldLists === 2) {
              entered()
              await blocked
              return Response.json({ jsonrpc: "2.0", id: msg.id, error: { code: -32603, message: "test tools unavailable" } })
            }
            const result =
    ...
    yield* provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        yield* mcp.add("example", cfg("user"))
        expect(Object.keys(yield* mcp.tools())).toEqual(["example_user"])
        release()
