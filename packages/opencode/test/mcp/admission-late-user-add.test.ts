// [TP-MCU-R7-21][TP-MCU-R10-20] Host admission: late user add cannot replace host-owned connection.
...
it.live("[TP-MCU-R7-21][TP-MCU-R10-20] late user add cannot replace newly host-owned connection", () =>
  Effect.gen(function* () {
    let release!: () => void
    let entered!: () => void
    let userLists = 0
    ...
    yield* provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const pending = yield* mcp.add("example", cfg("user")).pipe(Effect.forkChild)
