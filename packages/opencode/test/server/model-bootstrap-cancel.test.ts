...
import { createModelAPI } from "../../src/server/model-api"
...
describe("model API waiting for shared instance bootstrap", () => {
  for (const trigger of ["client cancellation", "listener close"] as const) {
    test(`${trigger} settles before an existing bootstrap is released`, async () => {
      ...
      const { token } = await LLMServerTokens.issue({
        directory: tmp.path,
        models: ["unused/chat"],
        expiry: { idleMs: 10000, maxAgeMs: 20000 },
      })
      const api = createModelAPI({ directory: tmp.path })
      const controller = new AbortController()
      const request = api.app.fetch(
        new Request("http://localhost/v1/chat/completions", {
          method: "POST",
          headers: { authorization: `Bearer ${token}`, "content-type": "application/json" },
          body: JSON.stringify({ model: "unused/chat", messages: [{ role: "user", content: "hello" }] }),
      ...
      try {
        ...
        expect(await completed?.[0].json()).toMatchObject({ error: { message: "Model request cancelled" } })
