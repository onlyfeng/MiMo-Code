...
- `reserved` - Compatibility buffer used with any output reservation to validate `max_context`; it is not subtracted from the compaction trigger.
...
The compaction trigger is `floor(effective window × ratio)`, with a default ratio of 90%
(configurable through `MIMOCODE_COMPACTION_TRIGGER_RATIO`). A valid `"max_context": "300K"`
budget therefore compacts at 270K by default. Reserves validate the budget but are not
subtracted from this trigger. Run
